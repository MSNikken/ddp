...
from .. import utils
from ..models.diffusion import ARInvModel
from ..models.helpers import cosine_beta_schedule, Losses, extract, apply_conditioning
...
class SE3Diffusion(nn.Module):
    ...
    def p_losses(self, x_start, cond, k, returns=None):
        ...
        H_rel_recon = pp.se3(x_recon[..., :6]).Exp()
        H_rel = H @ H_noisy.Inv()
        H_delta = (H_rel @ H_rel_recon.Inv()).Log()

        pose_loss, info = self.loss_fn(H_delta, torch.zeros_like(H_delta))
